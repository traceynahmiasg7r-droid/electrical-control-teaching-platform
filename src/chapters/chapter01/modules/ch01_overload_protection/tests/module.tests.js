...
const overloadOperation = instance.getOperationViewModel();
check("screenshot-style single QF control", overloadOperation.power.layout === "single-toggle");
check("operation buttons keep teaching colors", [
  ["SB1 启动", "forward"],
  ["SB2 停止", "stop"]
].every(([label, buttonClass]) => overloadOperation.controls.some((item) => item.visible && item.label === label && item.buttonClass === buttonClass))
  && overloadOperation.protection.visible
  && overloadOperation.protection.label === "FR1 过载");
